...
<form action="" method="post" class="form-horizontal needs-validation" novalidate>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <select class="form-select" name="a_masters_id">
        ...
        <option value="{{$a_item->id}}" @if($a_item->id==$item2->a_masters_id) selected @endif>
          {{$a_item->name}}
        </option>
        @endforeach
      </select>
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <input type="text" name="quantity" value="{{ $item2->quantity }}" class="form-control @if($errors->has('quantity')) is-invalid @endif" id="quantity" required>
